...
export default function Proveedores() {
  ...
  const { data: proveedores = [], isLoading } = useQuery({
    ...
    queryFn: () => workspace ? crm.entities.Proveedor.filter({ workspace_id: workspace.id }, "-created_date") : [],
    enabled: !!workspace
  });

  const { data: ventas = [] } = useQuery({
    queryKey: ['ventas-proveedores', workspace?.id],
    queryFn: () => workspace ? crm.entities.Venta.filter({ workspace_id: workspace.id }) : [],
    enabled: !!workspace
  });
