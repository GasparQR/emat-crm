...
export default function Postventa() {
  ...
  const { data: ventasFinalizadas = [], isLoading } = useQuery({
    ...
    queryFn: () => workspace
      ? crm.entities.Venta.filter({ workspace_id: workspace.id, estado: "Finalizada" }, "-fecha", 500)
      : [],
    enabled: !!workspace
  });
  ...
  const { data: contactos = [] } = useQuery({
    ...
    queryFn: () => workspace ? crm.entities.Contacto.filter({ workspace_id: workspace.id }) : [],
    enabled: !!workspace
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => crm.entities.Venta.update(id, data),
