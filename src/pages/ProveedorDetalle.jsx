...
export default function ProveedorDetalle() {
  ...
  const { data: proveedor, isLoading } = useQuery({
    ...
    queryFn: () => crm.entities.Proveedor.filter({ id: proveedorId }),
    select: (data) => data[0],
    enabled: !esNuevo
  });
  ...
  const { data: ventas = [] } = useQuery({
    ...
    queryFn: () => crm.entities.Venta.filter({ proveedorId }),
    enabled: !esNuevo
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => {
      if (esNuevo) {
        return crm.entities.Proveedor.create({ ...data, workspace_id: workspace?.id });
      }
      return crm.entities.Proveedor.update(proveedorId, data);
    },
    onSuccess: (data) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => crm.entities.Proveedor.delete(proveedorId),
    onSuccess: () => {
